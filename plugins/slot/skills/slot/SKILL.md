...
| "Spawn `claude-opus-5[1m]` so the subagent gets the 1M budget" | The Agent tool's `model` is an enum of family words (`sonnet`/`opus`/`haiku`/`fable`) — a full or suffixed id fails validation before spawn. Shipped in 1.4.0 on an untested assumption, reverted in 1.5.0. A Slot subagent's budget is not reachable from here. |
