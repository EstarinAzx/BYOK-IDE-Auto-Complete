...
Row 1 is the route: the Claude family **or the alias** the picked model resolved through (same lookup order as the Bridge — alias before family), the Wisp Target it is bound to, this conversation's context fill, and the Provider serving it. Pick an aliased Target (`sol`, `grok`, `gemini`) and the row names that alias and that Provider, not a Claude family. Below it, one bar per quota window the active Provider reported — percentage plus when it refills. The dimmed tail rows are other Providers the ledger remembers, stamped with how old the reading is; they are limits you are not spending right now, never presented as current.
...
Changing the block? Run `node plugins/slot/statusline/check.js` — it drives the real script through a sandboxed `WISP_HOME` and pins the resolution order (alias before family) plus the degrade ladder. No framework, exit code is the verdict.
